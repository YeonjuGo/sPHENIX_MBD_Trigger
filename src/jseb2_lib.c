...
//Modified - DO NOT UPLOAD THIS FILE LICENSE STRING
...
/* #define JSEB2_DEFAULT_LICENSE_STRING "12345abcde12345.abcde" */
#define JSEB2_DEFAULT_LICENSE_STRING "872759e7d000fa2756fb0e999c950ee5b6ca1d8d95b995d2e6ba7736dc95.WD1260_64_NL_Brookhaven_National_Laboratory"
#define JSEB2_DEFAULT_DRIVER_NAME WD_DEFAULT_DRIVER_NAME_BASE

/* JSEB2 device information struct */
typedef struct {
...
static CHAR gsJSEB2_LastErr[256];
...
static void DLLCALLCONV JSEB2_IntHandler(PVOID pData);
static void JSEB2_EventHandler(WD_EVENT *pEvent, PVOID pData);
...
static inline BOOL IsValidDevice(PWDC_DEVICE pDev, const CHAR *sFunc)
{
    ...
    {
        snprintf(gsJSEB2_LastErr, sizeof(gsJSEB2_LastErr) - 1, "%s: NULL device %s\n",
            sFunc, !pDev ? "handle" : "context");
        ErrLog(gsJSEB2_LastErr);
        return FALSE;
    }
 ...
 *************************************************************/
/* -----------------------------------------------
    JSEB2 and WDC library initialize/uninit
   ----------------------------------------------- */
DWORD JSEB2_LibInit(void)
{
    ...
    /* Set the driver name */
    if (!WD_DriverName(JSEB2_DEFAULT_DRIVER_NAME))
    {
    ...
    dwStatus = WDC_DriverOpen(WDC_DRV_OPEN_DEFAULT, JSEB2_DEFAULT_LICENSE_STRING);
    if (WD_STATUS_SUCCESS != dwStatus)
    {
...
WDC_DEVICE_HANDLE JSEB2_DeviceOpen(const WD_PCI_CARD_INFO *pDeviceInfo)
{
    ...
    if (!pDeviceInfo)
    {
        ErrLog("JSEB2_DeviceOpen: Error - NULL device information struct pointer\n");
        return NULL;
    }

    /* Allocate memory for the JSEB2 device context */
    pDevCtx = (PJSEB2_DEV_CTX)malloc(sizeof (JSEB2_DEV_CTX));
    if (!pDevCtx)
    {
        ErrLog("Failed allocating memory for JSEB2 device context\n");
        return NULL;
    }
    ...
    TraceLog("JSEB2_DeviceOpen: Opened a JSEB2 device (handle 0x%p)\n", hDev);
    return hDev;

Error:    
    if (hDev)
        JSEB2_DeviceClose(hDev);
    else
        free(pDevCtx);
...
BOOL JSEB2_DeviceClose(WDC_DEVICE_HANDLE hDev)
{
    ...
    PJSEB2_DEV_CTX pDevCtx;
    
    TraceLog("JSEB2_DeviceClose entered. Device handle: 0x%p\n", hDev);

    if (!hDev)
    {
        ErrLog("JSEB2_DeviceClose: Error - NULL device handle\n");
        return FALSE;
    }

    pDevCtx = (PJSEB2_DEV_CTX)WDC_GetDevContext(pDev);
    
    /* Disable interrupts */
    if (WDC_IntIsEnabled(hDev))
    {
        dwStatus = JSEB2_IntDisable(hDev);
        if (WD_STATUS_SUCCESS != dwStatus)
        {
    ...
    /* Free JSEB2 device context memory */
    if (pDevCtx)
        free (pDevCtx);
...
static void DLLCALLCONV JSEB2_IntHandler(PVOID pData)
{
    ...
    PJSEB2_DEV_CTX pDevCtx = (PJSEB2_DEV_CTX)WDC_GetDevContext(pDev);
    JSEB2_INT_RESULT intResult;
...
DWORD JSEB2_IntEnable(WDC_DEVICE_HANDLE hDev, JSEB2_INT_HANDLER funcIntHandler)
{
    ...
    TraceLog("JSEB2_IntEnable entered. Device handle: 0x%p\n", hDev);

    if (!IsValidDevice(pDev, "JSEB2_IntEnable"))
    ...
    /* Store the diag interrupt handler routine, which will be executed by
       JSEB2_IntHandler() when an interrupt is received */
    ...
    dwStatus = WDC_IntEnable(hDev, NULL, 0, 0,
        JSEB2_IntHandler, (PVOID)pDev, WDC_IS_KP(hDev));
    ...
    TraceLog("JSEB2_IntEnable: Interrupts enabled\n");
...
DWORD JSEB2_IntDisable(WDC_DEVICE_HANDLE hDev)
{
    ...
    TraceLog("JSEB2_IntDisable entered. Device handle: 0x%p\n", hDev);

    if (!IsValidDevice(pDev, "JSEB2_IntDisable"))
        return WD_INVALID_PARAMETER;
...
{
    if (!IsValidDevice((PWDC_DEVICE)hDev, "JSEB2_IntIsEnabled"))
        return FALSE;
...
static void JSEB2_EventHandler(WD_EVENT *pEvent, PVOID pData)
{
    ...
    TraceLog("JSEB2_EventHandler entered, pData: 0x%p, dwAction 0x%lx\n",
...
DWORD JSEB2_EventRegister(WDC_DEVICE_HANDLE hDev, JSEB2_EVENT_HANDLER funcEventHandler)
{
    ...
    TraceLog("JSEB2_EventRegister entered. Device handle: 0x%p\n", hDev);
    
    if (!IsValidDevice(pDev, "JSEB2_EventRegister"))
        return WD_INVALID_PARAMETER;
    ...
    /* Store the diag event handler routine to be executed from JSEB2_EventHandler() upon an event */
    pDevCtx->funcDiagEventHandler = funcEventHandler;

    /* Register event */
    dwStatus = WDC_EventRegister(hDev, dwActions, JSEB2_EventHandler, hDev, FALSE);
...
DWORD JSEB2_EventUnregister(WDC_DEVICE_HANDLE hDev)
{
    ...
    TraceLog("JSEB2_EventUnregister entered. Device handle: 0x%p\n", hDev);
    
    if (!IsValidDevice((PWDC_DEVICE)hDev, "JSEB2_EventUnregister"))
        return WD_INVALID_PARAMETER;
...
{
    if (!IsValidDevice((PWDC_DEVICE)hDev, "JSEB2_EventIsRegistered"))
        return FALSE;
...
DWORD JSEB2_GetNumAddrSpaces(WDC_DEVICE_HANDLE hDev)
{
    ...
    if (!IsValidDevice(pDev, "JSEB2_GetNumAddrSpaces"))
        return 0;
...
BOOL JSEB2_GetAddrSpaceInfo(WDC_DEVICE_HANDLE hDev, JSEB2_ADDR_SPACE_INFO *pAddrSpaceInfo)
{
    ...
    if (!IsValidDevice(pDev, "JSEB2_GetAddrSpaceInfo"))
        return FALSE;

#if defined(DEBUG)
    if (!pAddrSpaceInfo)
    {
        ErrLog("JSEB2_GetAddrSpaceInfo: Error - NULL address space information pointer\n");
        return FALSE;
    }
#endif
    ...
    {
        ErrLog("JSEB2_GetAddrSpaceInfo: Error - Address space %ld is out of range (0 - %ld)\n",
    ...
    if (WDC_AddrSpaceIsActive(pDev, dwAddrSpace))
    {
        ...
        snprintf(pAddrSpaceInfo->sDesc, MAX_DESC - 1, "0x%0*lX - 0x%0*llX (0x%llx bytes)",
...
static void ErrLog(const CHAR *sFormat, ...)
{
    ...
    vsnprintf(gsJSEB2_LastErr, sizeof(gsJSEB2_LastErr) - 1, sFormat, argp);
#if defined(DEBUG)
    WDC_Err("JSEB2 lib: %s", JSEB2_GetLastErr());
#endif
    va_end(argp);
}
